...
import type { VibeLabDiagramDefinition } from './VibeLabDiagramSchema';

export interface AssistToBuildReviewSummary {
  stage: 'as_is' | 'to_be';
  title: string;
  summary_markdown: string;
  diagram?: VibeLabDiagramDefinition | null;
  assumptions?: string[];
  risks?: string[];
  requirements?: string[];
}
...
export function AssistToBuildReviewCard({ approvalId, summary, onRespond, resolvedResponse }: {
  ...
  return <section className="bg-[var(--primary)]/5 border-2 border-[var(--primary)]/30 rounded-lg p-4" aria-label={`${isAsIs ? 'AS-IS' : 'TO-BE'} process review`}>
    ...
    {summary.diagram ? <div className="mb-3"><VibeLabDiagram code={JSON.stringify(summary.diagram)} /></div> : null}
