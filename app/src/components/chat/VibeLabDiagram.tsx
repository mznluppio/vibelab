...
export function VibeLabDiagram({ code }: VibeLabDiagramProps) {
  ...
  const download = (blob: Blob, extension: string) => {
    ...
    const safeTitle = data.title.toLowerCase().trim().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '') || 'diagram';
  ...
  const exportJson = () => download(new Blob([JSON.stringify(data, null, 2)], { type: 'application/json' }), 'json');
    ...
    <div className="relative">
      ...
      {exportOpen === variant && (
        <div className="absolute right-0 top-full z-20 mt-1 min-w-28 overflow-hidden rounded-md border border-[var(--border)] bg-[var(--surface)] p-1 shadow-lg">
          <button type="button" className="block w-full rounded px-2 py-1.5 text-left text-xs hover:bg-[var(--text)]/5" onClick={() => { exportJson(); setExportOpen(null); }}>Export JSON</button>
          <button type="button" className="block w-full rounded px-2 py-1.5 text-left text-xs hover:bg-[var(--text)]/5" onClick={() => { void exportPng(variant === 'inline' ? inlineCanvasRef.current : dialogCanvasRef.current); setExportOpen(null); }}>Export PNG</button>
        </div>
      )}
    </div>
  );
  ...
  return (
    <>
      ...
      >
        ...
        >
          <DiagramCanvas {...layout} expanded={false} onInstance={setInlineFlow} canvasRef={inlineCanvasRef} />
        </div>
      </section>
        ...
        <Dialog.Portal>
          ...
          >
            ...
            <div className="vibelab-diagram-dialog__canvas">
              <DiagramCanvas {...layout} expanded onInstance={setDialogFlow} canvasRef={dialogCanvasRef} />
            </div>
          </Dialog.Content>
        </Dialog.Portal>
      </Dialog.Root>
